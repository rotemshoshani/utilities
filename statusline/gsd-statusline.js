...
function runStatusline() {
  ...
  try {
    ...
    // 5-hour rate limit bar (cyan, mirrors context bar style)
    let fiveHr = '';
    const fiveHrPct = data.rate_limits?.five_hour?.used_percentage;
    if (fiveHrPct != null) {
      const used5 = Math.max(0, Math.min(100, Math.round(fiveHrPct)));
      const filled5 = Math.floor(used5 / 10);
      const bar5 = '█'.repeat(filled5) + '░'.repeat(10 - filled5);
      let resetStr = '';
      const resetsAt = data.rate_limits?.five_hour?.resets_at;
      if (resetsAt != null) {
        const resetsDate = new Date(resetsAt * 1000);
        const hh = resetsDate.getHours().toString().padStart(2, '0');
        const mm = resetsDate.getMinutes().toString().padStart(2, '0');
        resetStr = ` ${hh}:${mm}`;
      }
      fiveHr = ` \x1b[36m${bar5} ${used5}%${resetStr}\x1b[0m`;
    }

    // Output
    process.stdout.write(`\x1b[2m${model}\x1b[0m${ctx}${fiveHr}`);
  } catch (e) {
