    ...
    def test_load_config_reads_defaults_and_agent_commands(self) -> None:
        with self.subTest():
            ...
            with TemporaryDirectory() as raw_dir:
                ...
                self.assertEqual(config.agents[0].submit_keys, ("Enter",))
                self.assertEqual(config.agents[0].prompt_delivery, "paste")
                self.assertEqual(config.agents[1].prompt_delivery, "argument_file")
